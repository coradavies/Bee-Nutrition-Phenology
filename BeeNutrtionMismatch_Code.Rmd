...
Date listed in header is the date this script was last updated.
...
This R markdown document contains the analyses conducted for paper titled 
"Social but not solitary bee abundance tracks pollen protein accumulation in 
forest canopy gaps", published in Ecological Entomology (Davies & Davis, 2022).

## Calculate Bradford Standard Curves

Calculation of the Standard curves for Bradford Assay:

```{r standard-curves}
###Create model

#import data
Standard1 <-read.csv("Bradford/StandardCurve1.csv") 
#Contains concentration (ug/ml) and readings

Standard2 <-read.csv("Bradford/StandardCurve2.csv") 
#Contains concentration (ug/ml) and readings

Standard3 <-read.csv("Bradford/StandardCurve3.csv") 
#Contains concentration (ug/ml) and readings
...
#Visualize
ggplot(Standard1, aes(x = Concentration, y = Reading)) + geom_point() +
  stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1)

ggplot(Standard2, aes(x = Concentration, y = Reading)) + geom_point() + 
  stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1)

ggplot(Standard3, aes(x = Concentration, y = Reading)) + geom_point() + 
  stat_smooth(method = "lm", formula = y ~ poly(x, 2), size = 1)
...
# Solve for x=concentration for the readings using the equation generated from
# the linear model
...
Create a new data frame and save it as a csv so that it can be used 
in other analyses:
```{r protein-dataframe}
...
FloralProtein_Means<- FloralProtein_Means %>% 
  select(c(Species,Mass_Mean,
           Concentration_Mean,
           Count_Mean,
           FlowerMass_Mean,
           FlowerProtein_Mean))
...
The first objective is to analyze flower and bee phenology over a patch size 
gradient. 
...
```{r floral-abundance}
# Calculate summary data for floral abundance (average count/m^2) for each 
# species at each sampling interval 
...
# effects plot (for revision)
lines_m1<- predictorEffect("Size", m1_2)
S2a <- plot(lines_m1, 
            xlab="Canopy gap size (ha)",
	ylab=(expression(Floral~density~(count/m^2))),
	main="",
	confint=list(style="lines"),
	lines=list(col=c("gray", "black")))

# X-Y plot
ggscatter(AbunSum2,x="Size",y="Density",
                   palette=("#E69F00"),
              add="reg.line",
              conf.int =TRUE)+
  ylab(expression(Floral~density~(count/m^2)))+
...
# effects plot (for revision)
lines_m2<- predictorEffect("Size", m2_2)
S2b <- plot(lines_m2, 
            xlab="Canopy gap size (ha)",
...
# X-Y plot
ggscatter(Floralrich2,x="Size",y="Richness",
                   palette=("#E69F00"),
              add="reg.line",
              conf.int =TRUE)+
  ylab(expression(Floral~richness))+
...
# effects plot (for revision)
lines_m3<- predictorEffect("Size", m3_2)
S2c <- plot(lines_m3, 
            xlab="Canopy gap size (ha)",
	ylab="Floral diversity (Shannon H')",
	main="",
...
# X-Y plot
ggscatter(diversity,x="Size",y="Shannon",
                   palette=("#E69F00"),
              add="reg.line",
              conf.int =TRUE)+
  ylab("Floral diversity (Shannon H')")+
...
# effects plot (for revision)
lines_m4<- predictorEffect("Size", m4_2)
S2d <- plot(lines_m4, xlab="Canopy gap size (ha)",
	ylab=(expression(Log~site~protein~density~(ug/m^2))),
	main="",
	confint=list(style="lines"),
	lines=list(col=c("gray", "black")))
...
# X-Y plot
ggscatter(MeadowProtein,x="Size",y="ProteinDensitySum2",
                   palette=("#E69F00"),
              add="reg.line",
              conf.int =TRUE)+
  ylab(expression(Log~site~protein~density~(ug/m^2)))+
...
#visualize
fig3e<- ggplot(beenest_time, aes(x = as.factor(Date), y = IntTotal, fill = factor(Nest1, levels=c("above", "below", "both")))) + 
  geom_histogram(stat = "identity", position = "fill") +
  labs(fill = "Nesting") +
  xlab("Sample date") +
  ylab("Percent abundance (%)") +
...
chisq.test(beenest_meadowsize[3:5])
...
# Change 'introduced' to 'non-native'
index2$Status[index2$Status == "Introduced"] <- "Non-native" 
...
Fig6 <- index2 %>%
  ...
  geom_point(data=index2[index2$group == "important",], size=4,
             aes(color=Status)) +
  annotate(geom = 'text',
...
m8b <- lm(IntTotal ~ ProteinDensitySum2, 
            data=vars_I)
...
# Poisson
m9 <- glm(IntRich ~ FloralDensity + FloralRichness + ProteinDensitySum2 + Size + (1|Sample), 
           family="poisson",
            data=vars_I)
...
fig7a<- ggscatter(vars_I,x="ProteinDensitySum2",y="IntTotal",
  ...
  xlab(expression(Log~site~protein~density~(mu~g/m^2)))
  theme(legend.title = element_blank())
...
The following analyses examine bee functional traits across different meadows and floral protein availability. These analyses use body size (small, medium, large), sociality (social, soliatry, klepto, multiple), and nesting behavior (above ground, below ground, both). 

```{r bee-functional-traits}
...
```{r phenological-mismatch-data-clean}
###DATA PREPARATION###
...
Fig4A<- ggplot(acc, aes(x = Day, y = acc, color = Trend, linetype = Trend)) +
  stat_smooth(aes(fill= Trend), method="glm", se=TRUE, size = 1.5, alpha = .1,
  ...
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9", "#000000"), guide = "none") +
  theme_classic()
...
Fig4B<- ggplot(acc, aes(x = Day, y = acc, color = Trend, linetype = Trend)) +
  ...
  scale_linetype_manual(
    labels = c('Klepto', 'Multiple', 'Social', 'Solitary', 'Site Protein'), 
    values=c(1, 1, 1, 1, 4)) +
  scale_colour_manual(
    labels = c('Klepto', 'Multiple', 'Social', 'Solitary', 'Site Protein'), 
...
#Create blank data frame  
  ...
  newcol2 <- c(145, 154, 167, 180, 197, 208, 224, 232, 254,
               ...
               141, 153, 169, 181, 195, 210, 223, 233, 255,
               141, 153, 169, 181, 195, 210, 223, 233, 255)
  newcol3 <- c("above", "below", "both")
  #insert new columns by repeating each element in vector the required number of times
  df2 <- dplyr::mutate(df, Site = rep(newcol, each = 27),
                     Day = rep(newcol2, each = 3),
...
acc<- left_join(above2, below2, by=c("Site", "Day", "ProteinAcc"))
acc<- left_join(acc, both2, by=c("Site", "Day", "ProteinAcc"))
acc <- acc %>% pivot_longer(c(ProteinAcc, BeeAcc_2, BeeAcc_3, BeeAcc_4), names_to = "Trend", values_to = "acc")
acc <- acc %>% drop_na() #remove na rows
  
#plot
Fig4C<- ggplot(acc, aes(x = Day, y = acc, color = Trend, linetype = Trend)) +
  stat_smooth(aes(fill= Trend), 
              method="glm", se=TRUE, size = 1.5, alpha = .1,
                  method.args = list(family=binomial)) +
  ylab("Accumulation (%)") +
  scale_linetype_manual(labels = c('Above', 'Below', 'Both', 'Site Protein'), 
                        values=c(1, 1, 1, 4)) +
  scale_colour_manual(labels = c('Above', 'Below', 'Both', 'Site Protein'), 
                      values=c("#999999", "#E69F00", "#56B4E9", "#000000")) +
  scale_fill_manual(values=c("#999999", "#E69F00", "#56B4E9", "#000000"), 
                    guide = "none") +
  theme_classic()
...
```{r BODY SIZE: 33% temporal distance}
  #large
    #protein
...
```{r BODY SIZE: 50% temporal distance}
  #large
    #protein
...
```{r BODY SIZE: 90% temporal distance}
  #large
    #protein
...
```{r SOCIALITY: 33% temporal distance}
  #klepto
    #protein
...
fig5d<- ggplot(Mismatch_soc_33, aes(x=Sociality, y=mismatch, fill=Sociality)) +
  ...
  theme(axis.text.x=element_blank(),
      axis.ticks.x=element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+  
  stat_summary(geom = 'text', label = c("a","a","bc","ab"), fun = max, hjust=1, vjust=-0.3) + 
  scale_fill_manual(values=cbPalette)   
...
```{r SOCIALITY: 50% temporal distance}
  #klepto
    #protein
...
```{r SOCIALITY: 90% temporal distance}
  #klepto
    #protein
...
```{r NESTING: 33% temporal distance}
  ...
  #below
   ...
   #combine data 
  Mismatch_soc_33 <- inner_join(mismatch33_soc_2, mismatch33_soc_3, by = c("Site","Size"))
  Mismatch_soc_33 <- inner_join(Mismatch_soc_33, mismatch33_soc_4, by = c("Site","Size"))
  #rename columns
  Mismatch_soc_33 <- rename(Mismatch_soc_33, c(Above=Mismatch33_2, Below=Mismatch33_3, Both=Mismatch33_4))
  #wide to long format
  Mismatch_soc_33 = gather(Mismatch_soc_33, Nesting, mismatch, Above:Both, factor_key=TRUE)
...
fig5g<- ggplot(Mismatch_soc_33, aes(x=Nesting, y=mismatch, fill=Nesting)) +
  ...
  theme(axis.text.x=element_blank(),
      axis.ticks.x=element_blank(), panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"))+  
  stat_summary(geom = 'text', label = c("a","b","ab"), fun = max, hjust=1, vjust=-0.3) + 
  scale_fill_manual(values=cbPalette)   
    

```
h) 50% temporal distance
```{r NESTING BEHAVIOR: 50% temporal distance}
  #above
  ...
  #below
   ...
   #combine data 
  Mismatch_soc_50 <- inner_join(mismatch50_soc_2, mismatch50_soc_3, by = c("Site","Size"))
  Mismatch_soc_50 <- inner_join(Mismatch_soc_50, mismatch50_soc_4, by = c("Site","Size"))
  #rename columns
  Mismatch_soc_50 <- rename(Mismatch_soc_50, c(Above=Mismatch50_2, Below=Mismatch50_3, Both=Mismatch50_4))
  #wide to long format
  Mismatch_soc_50 = gather(Mismatch_soc_50, Nesting, mismatch, Above:Both, factor_key=TRUE)
...
```{r NESTING BEHAVIOR: 90% temporal distance}
  #above
  ...
  #below
   ...
   #combine data 
  Mismatch_soc_90 <- inner_join(mismatch90_soc_2, mismatch90_soc_3, by = c("Site","Size"))
  Mismatch_soc_90 <- inner_join(Mismatch_soc_90, mismatch90_soc_4, by = c("Site","Size"))
  #rename columns
  Mismatch_soc_90 <- rename(Mismatch_soc_90, c(Above=Mismatch90_2, Below=Mismatch90_3, Both=Mismatch90_4))
  #wide to long format
  Mismatch_soc_90 = gather(Mismatch_soc_90, Nesting, mismatch, Above:Both, factor_key=TRUE)
...
fig5i<- ggplot(Mismatch_soc_90, aes(x=Nesting, y=mismatch, fill=Nesting)) +
  ...
  stat_summary(geom = 'text', label = c("ab","a","b"), fun = max, hjust=1, vjust=-0.3) +
  scale_fill_manual(values=cbPalette)   
